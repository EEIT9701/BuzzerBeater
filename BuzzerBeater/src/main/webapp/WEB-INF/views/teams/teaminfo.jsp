...
<body>
	...
	<div class="container">
		<div class="jumbotron">
			...
			<!--上層導覽列(開始) -->
			<div id="pathWay">
				<span> <a href="<%=request.getContextPath()%>/index.jsp">
						<span>首頁</span>
				</a>
				</span>&gt; <span> <a
					href="<%=request.getContextPath()%>/teams/listAllteam_front.jsp">
						<span>球隊</span>
				</a>
				</span>&gt; <span><span>球隊資訊</span></span>
			</div>
			<!--上層導覽列(結束) -->
			<!-- 第二列(開始) -->
			<!--****************-->
					...
					<div id="tabs">
								...
								<table class="table table-bordered respond"  style="table-layout:fixed">
									...
									<tbody>
										<tr align='center' valign='middle'>
											...
											<td><c:if
													test="${null==teamCompositionVO.playersVO.photo}">
													<img id="photo"
														src="<%=request.getContextPath()%>/images/placholder_testimonial-180x180.png">
												</c:if> <c:if test="${null!=teamCompositionVO.playersVO.photo}">
													<img id="photo"
														src="data:image/jpeg;base64,${teamCompositionVO.playersVO.photo}">
												</c:if></td>
											<td><a
												href="<%=request.getContextPath()%>/PersonalData.do?action=Get_singleData&playerID=${teamCompositionVO.playersVO.playerID}">${teamCompositionVO.playersVO.playerName}</a></td>
						...
						<div id="tabs-2">
							<table class="table table-bordered" align='center'
								valign='middle  style="table-layout:fixed"'>
								<thead>
									<tr>
										<th>比賽時間</th>
										<th>比賽地點</th>
									...
									<c:forEach var="gamesVO" items="${gamesVO}">
										<tr align='center' valign='middle'>
											...
											<td><fmt:formatDate type="both" dateStyle="short"
													timeStyle="short" value="${gamesVO.gameBeginDate}" /></td>
											<td><a href="<%=request.getContextPath()%>/Locationinfo.do?action=GET_ONE_LOCATION&locationID=${gamesVO.locationinfoVO.locationID}">${gamesVO.locationinfoVO.locationName}</a></td>
											<td><c:if
													test="${teamsVO.teamID == gamesVO.teamAVO.teamID}">
							               <a href="<%=request.getContextPath()%>/Teams.do?action=GET_ONE_TEAM&teamID=${gamesVO.teamBVO.teamID}">${gamesVO.teamBVO.teamName}</a>
									    </c:if> <c:if test="${teamsVO.teamID == gamesVO.teamBVO.teamID}">
							               <a href="<%=request.getContextPath()%>/Teams.do?action=GET_ONE_TEAM&teamID=${gamesVO.teamAVO.teamID}">${gamesVO.teamAVO.teamName}</a>
									    </c:if></td>
											<td><c:if
													test="${teamsVO.teamID == gamesVO.teamAVO.teamID}">
							               ${gamesVO.teamAScore}- ${gamesVO.teamBScore}
									    </c:if> <c:if test="${teamsVO.teamID == gamesVO.teamBVO.teamID}">
							              ${gamesVO.teamBScore}- ${gamesVO.teamAScore}
									    </c:if></td>
									    <td><c:if test="${teamsVO.teamID == gamesVO.teamAVO.teamID}">
							               <c:if test="${gamesVO.teamAScore > gamesVO.teamBScore}">
							               勝
									    </c:if> 
									    <c:if test="${gamesVO.teamAScore < gamesVO.teamBScore}">
							               負
									    ...
									    <c:if test="${teamsVO.teamID == gamesVO.teamBVO.teamID}">
							               <c:if test="${gamesVO.teamBScore > gamesVO.teamAScore}">
							               勝
									    </c:if> 
									    <c:if test="${gamesVO.teamBScore < gamesVO.teamAScore}">
							               負
